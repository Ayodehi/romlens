...
| D2 comparing two ROMs in the app | done, 26 September 2026: File › Compare With… (⌥⌘D) opens another version, a ROM file or a saved project (its ROM found as opening it would find it, its names coming with it), analyses it and compares it with this one in a Compare tab, which the toolbar offers only while comparing. The list gives the routines that are not the same (changed, moved, added, removed), the data regions with changed bytes, the moved blocks and the first thousand stretches of bytes that differ; a changed routine shows both versions' instructions side by side, aligned by the pairing and tinted by what changed, with Show in Listing; the rest go to the hex. Carry Over N Names names the paired routines the other version names and this one does not, as one undo step (`diff::names_to_carry`, `Workbench::carry_names`); the comparison follows every edit. FFI: `Workbench::compare_with` (async, and `_blocking`), `ComparisonInfo`. Tests: Rust, an FFI test (carry and undo) and two app tests (106 pass) |
| L1 stack-relative locals and arguments | done, 26 September 2026. A survey of the user's 44 dumps under ~/Documents/SD2SNES found 375 of 21,693 routines addressing the stack by offset (Sim City 46, Actraiser 48, Final Fantasy III 65, Super Metroid 9); the C showed each as RAM (`ADDR_7E0002[S]`). Now `stack_slots` follows which push put each byte on the stack through the graph, as it did for pairing pushes with pulls: `LDA $03,S` reading or writing what the routine pushed is that push's temporary (a byte of it where narrower, `t = (t & 0xFF) \| v << 8` for a write), and `LDA ($01,S),Y` reads through a pushed pointer (Sim City's tables fold to `MEM16(0x8072)`-style reads). Bytes above the return address (two for `RTS`, three for `RTL`) are arguments: each piece the routine reads is `arg3`, `arg5`, … read once at entry from `STACK16(S + 1)`, since a call in C leaves no return address; the summary says "Reads 2 bytes its caller pushed", and a caller's push that a callee reads stays a push (`Program::stack_args`, `Conventions::stack_args`). What cannot be followed keeps the stack as memory, printed `STACK8`/`STACK16(S + k)` (new in `snes.h`, bank 0) rather than as RAM, with a note giving the reason; after L1, 246 of the 375 read as variables, and of the 129 left 51 read their own return address (data placed after the call), 23 pull more than they push, 17 leave bytes pushed, 11 move S with something pushed, 11 write their arguments. The differential harness now keeps the stack apart from the memory a routine reads, as the CPU's page is, and its lift level reads the stack through `STACK8`/`16`; run on 18 games with no new differences (every one left was checked against the code before L1 and is an older full-level bug, listed under M). Found on the way and fixed: a block the structure prints twice took its label twice (Final Fantasy III's C did not compile); `(void)` before an operator; a large constant plus S (`TSX` then `DEC $B200,X`) is not the stack. At the `full` level a routine whose every caller can pass them, and that nothing reaches unseen, takes its arguments as parameters: `PEA $1234; JSR SUB_008060; PLA` reads `SUB_008060(0x1234);` and the routine `void SUB_008060(u16 arg3)`. The caller's pushes are temporaries carried into the call (a `stack_args` line the call takes); where they cannot be, the call passes the bytes on the stack. The clean level keeps the pushes and the reads at entry. For the arguments the harness gives each run a caller's pushed bytes, runs the lift level of an argument-reading routine past a return address as the CPU does, and leaves out routines that pull more than they pushed or read their return address (`uses_call_frame`), which no C can match; breaking the argument offset on purpose fails the new stack fixture's differential test. The runs on real games found three more, fixed: an argument read in overlapping pieces (`LDA $05,S` and `LDA $06,S`, The Lion King) takes a byte temporary each; where the slots cannot be followed the memory form is shifted by the return address, as C sees the stack; and a routine a jump table, pointer or vector reaches takes no parameters, since those callers pass nothing. The harness's alarm also ran while a child printed what it wrote, so a routine writing much memory was cut off mid-line (Final Fantasy III); it stops when the routine returns |
| S1 ca65 `.dbg` import | done, 26 September 2026: `io::import::dbg` reads version 2 of the debug information ld65 writes with `--dbgfile`. Symbols that are labels become labels through `symbols::plan`, as the other symbol files do: a ROM symbol at the place its segment was written in the output (`ooffs` plus its offset in the segment, so a LoROM `start = $8000` and a `$808000` both place), RAM ones at their address (`$7E:0200`); scopes qualify names (`Player::Draw`, `ClearBuffer@loop`, sanitized to `ClearBuffer_loop`), a routine's name wins over a cheap local at its first byte, and equates and imports are counted, not labelled. Lines become a `model::source_map::SourceMap` (the files, and each line that made bytes with its ROM ranges and kind, the program's own, a macro's once per expansion, or another tool's), kept in the project as `imports/sources.json`, one per `.dbg`, with the folder it came from so the sources are found beside it. The output file is the one named like the ROM, else the largest; a `.dbg` whose segments were written elsewhere places nothing and says so. `romlens import dbg <project> <dbg>`, and `romlens source <rom> --project P` with an address (the lines that made it, the program's first), `--line FILE:LINE` (the bytes it made) or neither (the files, and which are missing). The fixture (`romlens-core/tests/data/ca65`: two modules, a macro file, zero page, BSS, RODATA and the header) is built by its `build.sh` with Homebrew's cc65 2.18 and committed with its output, so CI needs no toolchain; building it found that a module assumed 8-bit index registers where the caller had set 16 (each ca65 module starts from its defaults), which the analysis showed as code running into the filler. Tests: five on the fixture (labels, lines, a macro's two expansions, the project round trip, a `.dbg` that places nothing) and a golden |
| S2 the Source view | done, 26 September 2026: File › Import › ca65 Debug Information… reads a `.dbg` (`Workbench::import_dbg`: the labels one undo step, the lines kept like a trace) and opens a Source tab, which the toolbar and View › Source offer only while the project has sources. The tab is the source on the left, a pop-up of the files, each line numbered and the lines that made bytes in full colour, and the disassembly on the right. Clicking a line selects the bytes it made and scrolls the listing there; a macro's line made bytes once per expansion, and clicking it again goes to the next. Selecting bytes anywhere highlights the line that made them and scrolls to it, the program's own line before the macro's, switching to its file (a file chosen by hand stays until the selection moves). The app is sandboxed, so the import asks once for the folder holding the sources and keeps a security-scoped bookmark for it in user defaults, not in the project; a file it cannot read, or one whose size differs from when it was assembled, says so, with Allow Access… to choose the folder again. FFI: `source_files`, `source_lines_at`, `source_file_lines`, `make_ca65_test_program`; `API_VERSION` 0.10.0. Tests: an FFI test, three app tests (109 pass) |
| T1 the tutor's client, key and tool loop | to do |
| T2 Ask mode | to do |
| T3 Fix mode | to do |
| T4 Investigate mode | to do |
| M measure and record | to do. Older full-level differences to look into, found by the L1 runs and present before it: Actraiser $00:8241, $02:C127; Sim City $00:B743, $00:D199, $01:8E3D, $01:A660, $01:AE26, $01:E8E9, $02:A6E7, $02:A785, $02:A7A2; Chrono Trigger $C0:1985; Final Fantasy Mystic Quest $0C:8AD2; Populous $00:AF99; F-Zero $00:EE93; Super Mario Kart $C0:816D, $C1:CB98; Fatal Fury 2 $C0:03C6; Super Ghouls'n Ghosts $01:96CA; Subversion $80:9A79, $86:8125; The Lion King $C0:A06E; Final Fantasy III, 30 routines, all in bank C2 (at the full level $C2:0E77, 2C30, 372F, 5FEF, 606D, EA43, EEAC, EEFE, EF4B, F392, F418, F5C1, F5F8, F630, F63E; in the summary checks $C2:2B30, 30DC, 54DC, 5723, 6170, 6174, 6178, 6283, 6293, C081, EDAC, EF1C, F997, F9C7, F9DF). And a full-level routine at an address outside a routine's unit prints its callees' prototypes but calls them without arguments (Actraiser $00:D016) |

...
| G7 measure and record | done, 24 September 2026: on the development ROM (Super Metroid, static analysis only) 799 routines, 5,077 blocks, 6,299 edges, the largest 82 blocks, no box on another, the slowest layout 0.3 ms, and a graph plus its neighbourhood for every routine in 2.4 s in all (the neighbourhood rediscovers every routine, about 3 ms each). Super Mario World with its project and a live session's log: RESET is 5 blocks and 2 loops, the clearing loop run 768 times and the main loop's wait 3.6 billion |

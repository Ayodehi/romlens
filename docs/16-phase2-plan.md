...
| 2B previews (2.25) | done: `RegionParams` (palette, columns, screen size, and `tiles` — the ROM address a tilemap preview draws with, in place of the sketched VRAM character base, which ROM bytes do not have) on the override, set by an undoable `SetRegionParams` that never re-runs the analysis; `viewmodel::preview`; `romlens project <P> preview` and a `Preview:` line in `inspect` |
| 2B FFI | done: the decoders as free functions over bytes (`Rom::bytes` or `RecordingSession::region` feed them alike), images as one RGBA buffer, `tile_bit_sources` fetched once per format, `RecordingSession`, `SetRegionParams`, `preview_at` |
| 2B macOS shell | done: a Graphics picker beside the three text tabs (⌥⌘4–7) opening the Tile Decoder, Palette, OAM and Tilemap views; clicks select the byte range they came from, so the editor shares the selection; File › Open Recording… with a frame field and prev/next; the inspector's Preview section with "Open in …" and "Decompress and Open". `GraphicsShellTests` covers it; the manual pass is still to run |
